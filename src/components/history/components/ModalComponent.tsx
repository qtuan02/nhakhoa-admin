...
import { addService, getHistoryState } from "@/redux/reducers/historyReducer";
...
export default function ModalComponent({ modal, toggle }: IModalComponentProps) {
    ...
    return (
        <CModal open={modal} onCancel={() => toggle()} footer={null} width={800}>
            <Divider><CTitle level={4}>Chọn dịch vụ</CTitle></Divider>
            <CSkeleton loading={service.loading}>
                <CTable
                    className="table-modal"
                    columns={columns}
                    dataSource={service?.data?.map((item: IService, index: number) => ({ ...item, index: index + 1, key: item.id, price: item.min_price, quantity: 1 })) || []}
