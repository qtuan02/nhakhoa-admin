...
import { getUserState, toggleModal } from "@/redux/reducers/userReducer";
...
export default function ModalComponent() {
    ...
    const handleSubmit = async (value: IUser) => {
        setLoading(true);
        const res = await changePassword(user.user_id as string, value.password as string);
        setLoading(false);
        if(res) {
    ...
    return (
        <Modal title={"Đổi mật khẩu"} open={user.modal} onCancel={handleCancel} footer={null}>
