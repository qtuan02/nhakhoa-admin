...
export default function TableComponent() {
    ...
    const columns: TableColumnsType<ICategory> = [
        ...
        },
        {
            ...
            render: (item) => (
                <CSpace>
                    <CButton link={`/danh-muc/sua/${item.id}`} type="primary" icon={<FontAwesomeIcon icon={faPenToSquare} />} className='ts-16'></CButton>
    ...
    return (
        <CTable
            columns={columns}
            dataSource={category.data?.map((item, index) => ({...item, index: index + 1, key: item.id }))}
